...
fit_given_k <- function(data, sp, sigma, k, fit_km1, basis) {
    ...
    opt_out <- optim(par_init, find_loglikelihood_k, X_k = X_k, fit_km1 = fit_km1)

    alpha_k <- opt_out$par
    f_k <- X_k %*% alpha_k
    cluster_info <- lapply(fit_km1$cluster_info, f_k = f_k)
...
fit_0 <- function(data, sp, sigma, basis) {
    ...
    cluster_info <- lapply(clusters, init_cluster_info)
...
init_cluster_info <- function(cluster, data, sigma) {
    rows <- which(data$c == cluster)
    n_c <- length(rows)
    Sigma <- diag(sigma^2, nrow = n_c, ncol = n_c)
    Sigma_inv <- 1 / Sigma
    ldet_Sigma <- 2 * n_c * log(sigma)
    z <- mod_0$residuals[rows]
